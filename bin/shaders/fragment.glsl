...
void main()
{
  // --- Compute TBN manually.
    ...
    // Adjust brightness factor if needed.
    float brightnessFactor = 2.0;  // Increase to test brighter output.

    // Loop over lights.
    for (int i = 0; i < numLights; i++) {
        if (length(lights[i].color) < 0.001)
            continue;
        lighting += brightnessFactor * computeLightContribution(lights[i], fs_in.wPosition, normal, viewDir, metallic, roughness, ao, baseColor);
    }
